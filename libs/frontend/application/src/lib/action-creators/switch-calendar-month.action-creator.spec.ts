...
describe('SwitchCalendarMonthActionCreator', () => {
    it('creates action', () => {
        const testingModule = createTestingModule({
            providers: [SwitchCalendarMonthActionCreator, Dispatcher],
        });
        ...
        expect(dispatcher.emit).toHaveBeenNthCalledWith(2, expect.any(ChangeCalendarDaysAction));
        expect(dispatcher.emit).toHaveBeenNthCalledWith(3, expect.any(ChangeCalendarMonthAction));
    });
});
